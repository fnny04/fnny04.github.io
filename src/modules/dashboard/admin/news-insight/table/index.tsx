...
import { useDeactivatedArticle, useDeleteArticle, useGetArticles } from "../hook";
import { Button, DataTable, Modal, FieldToogle, Toast } from "@/components";
import { Icon } from "@iconify/react/dist/iconify.js";
import { TArticleItem } from "../type";
import { useRouter } from "next/navigation";
export const TableArticles: FC = (): ReactElement => {
  const { data: articles, refetch, isLoading } = useGetArticles();
  const { mutate: activatedData } = useDeactivatedArticle();
  const { mutate: deleteData } = useDeleteArticle();
  ...
  const columns = useMemo<ColumnDef<Unpacked<TArticleItem>>[]>(
    () => [
      {
        header: "No",
        accessorKey: "no",
        ...
        cell: ({ row }) => {
          return (
            <div className="flex items-center gap-x-2" key={row.original.id}>
              <Button
                onClick={() => {
                  router.push(`/dashboard/news-insight/${row.original.slug}`);
                }}
                type="button"
  ...
  return (
    <>
      <section className="flex w-full h-full min-h-screen">
        ...
        <DataTable
          isLoading={isLoading}
          redirect={() => router.push("/dashboard/news-insight/create")}
          columns={columns}
          handleSearch={handleSearch}
          data={
            articles?.filter((x) => x.title.toLowerCase().includes(searchQuery.toLowerCase())) || []
          }
        />
      </section>
